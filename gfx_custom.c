...
//CUSTOM functions
int current_xpos(void){ //returns the current xpos of the mouse without an event happening
    int rootx, rooty, childx, childy;
    Window inwin, inchildwin;
    unsigned int mask;
    XQueryPointer(gfx_display, gfx_window, &inwin, &inchildwin, &rootx, &rooty, &childx, &childy, &mask);
    return childx;
}

int current_ypos(void){ //returns the current ypos of the mouse without an event happening
    int rootx, rooty, childx, childy;
    Window inwin, inchildwin;
    unsigned int mask;
    XQueryPointer(gfx_display, gfx_window, &inwin, &inchildwin, &rootx, &rooty, &childx, &childy, &mask);
